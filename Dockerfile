...
# Start gateway server. Config now lives in the bind-mounted /home/node/.openclaw
# directory (see Coolify Storages). --allow-unconfigured lets the gateway boot
# even when the bind-mount target is empty on first deploy.
#
